...
# Setup page config
st.set_page_config(
    page_title="Multi-Stage Hallucination Mitigation RAG",
    page_icon="🛡️",
    layout="wide"
)
...
st.markdown("<hr style='border: 0; height: 1px; background: linear-gradient(to right, rgba(99,102,241,0), rgba(99,102,241,0.5), rgba(99,102,241,0)); margin-bottom: 25px;'>", unsafe_allow_html=True)

# Sidebar Configuration panel
st.sidebar.markdown('<h2 style="font-family: \'Space Grotesk\'; font-weight: 600; color: #f1f5f9 !important;">🛡️ Navigation</h2>', unsafe_allow_html=True)
menu = st.sidebar.radio("Go To", ["💬 Chatbot Assistant", "📈 Performance Graphs", "📤 Ingest Knowledge Base"])

st.sidebar.markdown("<hr style='border-color: rgba(255,255,255,0.05);'>", unsafe_allow_html=True)

if menu == "💬 Chatbot Assistant":
    st.sidebar.markdown('<h3 style="font-family: \'Space Grotesk\'; font-weight: 600; color: #f1f5f9 !important; font-size: 1.1rem; margin-top:0px;">⚙️ Pipeline Settings</h3>', unsafe_allow_html=True)
    pipeline_version = st.sidebar.selectbox(
        "Select Framework Version",
        ["V0 (Basic RAG)", "V1 (Web Fallback)", "V2 (Reranking)", "V3 (Claim Verification)", "V4 (Confidence Scoring)", "V5 (Query Intelligence)"],
        index=0
    )
...
if menu == "💬 Chatbot Assistant":
    ...
    with main_chat:
        with st.container(border=True):
            st.markdown('<h3 style="font-family: \'Space Grotesk\'; font-weight: 600; margin-top:0px; color: #f8fafc !important;">💬 Chat Assistant</h3>', unsafe_allow_html=True)
                    ...
                    if chat["role"] == "user":
                        ...
                        with st.chat_message("assistant", avatar="🛡️"):
                            ...
                            if chat.get("decision") == "ESCALATE":
                                st.markdown(f"<span style='color: #fca5a5;'>⚠️ {chat['content']}</span>", unsafe_allow_html=True)
                            else:
                                st.markdown(chat["content"])
                                
                            # Mini metric badges below assistant responses
                            if "confidence_score" in chat:
                                status_class = "badge-accept" if chat["decision"] == "ACCEPT" else "badge-escalate"
        ...
        # Chat Input Box
        user_input = st.chat_input("Ask a question about academics, placements, admissions, hostels...")
        ...
        if user_input:
            ...
            with st.spinner("Generating reliable response..."):
                try:
                    ...
                    st.error(f"Failed to connect to FastAPI backend: {str(e)}. Make sure `uvicorn main:app` is running.")

    # Doc Viewer Sidebar column (Right-hand metrics and sources panels)
    with doc_viewer:
        with st.container(border=True):
            st.markdown('<h3 style="font-family: \'Space Grotesk\'; font-weight: 600; margin-top:0px; color: #f8fafc !important; font-size: 1.25rem; display: flex; align-items: center; gap: 8px;"><span style="color: #6366f1;">📊</span> Pipeline Analytics</h3>', unsafe_allow_html=True)
            st.markdown('<p style="color: #94a3b8 !important; font-size: 0.8rem; margin-bottom: 20px; line-height: 1.3;">Real-time checks of relevance, faithfulness, and grounding sources.</p>', unsafe_allow_html=True)
            

            
            if st.session_state.chat_history:
                # Get last assistant result
                last_response = [c for c in st.session_state.chat_history if c["role"] == "assistant"]
                if last_response:
                    ...
                    conf_color = "#34d399" if conf_val >= 0.70 else "#fbbf24" if conf_val >= 0.50 else "#f87171"
                    st.markdown(f"""
                    ...
                    if latest.get("raw_answer") != latest.get("answer"):
                        with st.expander("🔍 View Raw Generated Response"):
                            st.caption("This was the initial answer before claim verification filtered unsupported claims:")
                            st.write(latest["raw_answer"])
                    
                    # Source Documents display
                    st.markdown('<h4 style="font-family: \'Space Grotesk\'; margin-top: 10px; color: #e2e8f0 !important; font-size: 1.0rem; font-weight: 600; margin-bottom: 6px; display: flex; align-items: center; gap: 6px;">📄 Grounding Sources</h4>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown('<h3 style="font-family: \'Space Grotesk\'; font-weight: 600; margin-top:0px; color: #f8fafc !important;">📊 Framework Version Evaluation</h3>', unsafe_allow_html=True)
        
        st.write("This dashboard displays the comparative evaluation metrics (Retrieval Relevance, Answer Relevance, and Faithfulness) across all six incremental versions of the framework (V0 to V5), as established in the M.Tech project research methodology.")

        col_btn, col_info = st.columns([0.4, 0.6])
        with col_btn:
            if st.button("🔄 Run Live Golden Batch Evaluation"):
                with st.spinner("Executing golden dataset queries across V0-V5... (Approx 45s)"):
                    try:
                        for v in ["v0", "v1", "v2", "v3", "v4", "v5"]:
                            run_evaluation_for_version(v)
                        st.success("Batch evaluation completed and CSV metrics updated successfully!")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Batch evaluation failed: {str(e)}")
        with col_info:
            st.caption("Running the batch evaluation invokes the golden dataset queries (across placements, hostels, admissions, etc.) sequentially through all 6 framework versions and logs the metrics to a CSV file.")
        ...
        try:
            ...
            if graph_option == "Combined Metrics Comparison":
                ax.plot(summary["Version"], summary["Retrieval_Relevance"], marker='o', label="Retrieval Relevance (RR)", color="#818cf8", linewidth=2.5)
                ax.plot(summary["Version"], summary["Answer_Relevance"], marker='s', label="Answer Relevance (AR)", color="#34d399", linewidth=2.5)
                ax.plot(summary["Version"], summary["Faithfulness"], marker='^', label="Faithfulness (F)", color="#f87171", linewidth=2.5)
                title = "Performance Metrics Trends Across Framework Versions (V0 - V5)"
            elif graph_option == "Figure 3: Retrieval Relevance (RR) Across Framework Versions":
                ax.plot(summary["Version"], summary["Retrieval_Relevance"], marker='o', label="Retrieval Relevance (RR)", color="#818cf8", linewidth=2.5)
                title = "Figure 3: Retrieval Relevance Across Framework Versions"
            elif graph_option == "Figure 4: Answer Relevance (AR) Across Framework Versions":
                ax.plot(summary["Version"], summary["Answer_Relevance"], marker='s', label="Answer Relevance (AR)", color="#34d399", linewidth=2.5)
            ...
            ax.legend(facecolor="#1e293b", edgecolor="none", labelcolor="#cbd5e1", fontsize=10)
    ...
    with st.container(border=True):
        st.markdown('<h3 style="font-family: \'Space Grotesk\'; font-weight: 600; margin-top:0px; color: #f8fafc !important;">📤 Ingest Knowledge Base</h3>', unsafe_allow_html=True)
        st.write("Upload institutional documents and FAQ datasets to build or update the RAG system's knowledge base. New uploads are processed, chunked, embedded using local Hugging Face embedding models, and persisted to the local Chroma Vector Database.")
        
        col_pdf, col_csv = st.columns(2)
        with col_pdf:
            st.markdown('<h4 style="font-family: \'Space Grotesk\'; color: #818cf8 !important; font-weight: 600;">📂 Institutional PDFs</h4>', unsafe_allow_html=True)
            uploaded_pdfs = st.file_uploader("Upload PDFs (e.g. seat_handbook.pdf)", type=["pdf"], accept_multiple_files=True, key="ingest_pdfs")
        with col_csv:
            st.markdown('<h4 style="font-family: \'Space Grotesk\'; color: #34d399 !important; font-weight: 600;">📂 FAQ Datasets</h4>', unsafe_allow_html=True)
            uploaded_csvs = st.file_uploader("Upload CSVs (e.g. seat_faq_dataset.csv)", type=["csv"], accept_multiple_files=True, key="ingest_csvs")
        ...
        if st.button("🚀 Process & Load Knowledge Base", use_container_width=True):
            if not uploaded_pdfs and not uploaded_csvs:
                ...
                with st.spinner("Processing uploads & building database... (This involves semantic chunking, local embeddings, and vector DB indexing)"):
                    try:
                        req_files = []
                        if uploaded_pdfs:
                            for pdf in uploaded_pdfs:
                                req_files.append(("pdfs", (pdf.name, pdf.getvalue(), "application/pdf")))
                        if uploaded_csvs:
                            for csv in uploaded_csvs:
                                req_files.append(("csvs", (csv.name, csv.getvalue(), "text/csv")))
                                
                        res = requests.post(f"{API_URL}/ingest", files=req_files)
                        if res.status_code == 200:
                            st.success("🎉 Ingestion successful! Knowledge base reloaded and Chroma DB indexed successfully.")
                        else:
